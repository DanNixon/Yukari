...
typedef void (*MSP_Ident)(float_t version, uint8_t multiType, uint8_t mspVersion,
                          uint32_t capability);

typedef void (*MSP_Status)(uint16_t cycleTime, uint16_t i2cError, uint16_t activeSensors,
                           uint32_t mode, uint8_t profile);

typedef void (*MSP_SensorValues)(uint16_t accelerometer[3], uint16_t gyroscope[3],
...
typedef void (*MSP_GPSRawData)(uint8_t fix, uint8_t numSat, uint32_t lat, uint32_t lon,
                               uint16_t alt, uint16_t speed, uint16_t groundCourse);

typedef void (*MSP_GPSComputedData)(uint16_t distanceToHome, uint16_t directionToHome,
                                    uint8_t update);
